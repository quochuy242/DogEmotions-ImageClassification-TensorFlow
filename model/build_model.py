...
early_stopping = EarlyStopping(
    monitor="val_loss", mode="min", restore_best_weights=True, patience=100
)
model_checkpoint = ModelCheckpoint(
    filepath="best_model.keras", monitor="val_loss", save_best_only=True, mode="min"
)
...
def create_model(resize_shape: list = [32, 32], initial_learning_rate: float = 0.0001):
    model = Sequential()
    model.add(Rescaling(1.0 / 255, input_shape=(384, 384, 3)))
...
def fit_model(model, train, val, epochs: int = 1000):
    ...
    load_model = tf.keras.models.load_model("best_model.keras")
